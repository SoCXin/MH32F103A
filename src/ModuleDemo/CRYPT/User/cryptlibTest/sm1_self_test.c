...
//    memset(cau8Plain, 'd', sizeof(cau8Plain));
    ...
    MHRAND_Prand(&callSm1.u32Rand, sizeof(callSm1.u32Rand));
...
void SM1_128_FuncTest()
{
    ...
    callSm1.pu8IV = au8Iv;
    callSm1.pu8Key = au8Key;
    callSm1.u16Opt = MH_SM1_OPT_MODE_ENCRYPT | MH_SM1_OPT_KEY_128 | MH_SM1_OPT_BLK_CBC | MH_SM1_OPT_ROUND_8;
    callSm1.u32Crc = MHCRC_CalcBuff(0xffff, &callSm1, sizeof(MH_SYM_CRYPT_CALL)-4);
    MHSM1_EncDec(&callSm1);
    ouputRes("SM1 CBC.\n", au8Cipher128_8, 16);
...
void SM1_Rand_Test(void)
{
    ...
    MH_SYM_CRYPT_CALL callSm1;  
    SM1_MODE_TYP ModeType[2] = {{MH_SM1_OPT_BLK_ECB, "ECB"},
                             {MH_SM1_OPT_BLK_CBC, "CBC"}};
    SM1_KEY_TYP KeyType[3] = {{32, NULL, MH_SM1_OPT_KEY_128, "KEY_128"},
                           {40, NULL, MH_SM1_OPT_KEY_192, "KEY_192"},
                           {48, NULL, MH_SM1_OPT_KEY_256, "KEY_256"}};
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        for (j = 0; j < 2; j++)
        {
            ...
            for(k = 0; k < 4; k++)
            {
                ...
                callSm1.u32Crc = MHCRC_CalcBuff(0xffff, &callSm1, sizeof(MH_SYM_CRYPT_CALL)-4);               
                if(MH_RET_SM1_SUCCESS != MHSM1_EncDec(&callSm1))
                {
...
uint32_t SM1_Check(void)
{
    ...
    call.u32InLen = 16;
    call.u32OutLen = 16;
    call.u16Opt = MH_SM1_OPT_MODE_ENCRYPT | MH_SM1_OPT_KEY_128 | MH_SM1_OPT_BLK_CBC | MH_SM1_OPT_ROUND_8;
    call.u32Crc = MHCRC_CalcBuff(0xffff, &call, sizeof(call)-4);
