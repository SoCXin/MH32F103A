...
double mh_frequency(uint8_t *, uint32_t);
double mh_block_frequency(uint8_t *, uint32_t);
double mh_runs(uint8_t *, uint32_t);
double mh_longest_run_of_ones(uint8_t *, uint32_t);
...
//    uint8_t s[] = {0x79, 0x20, 0xDE, 0x0E,0x6E,0x22,0xFD,0xB7,0x82,0x92,0x9C,0x12,0xA4,0x4A,0x0F,0x22};
    ...
    double (*mh_rand_func[4])(uint8_t *s, uint32_t n) = {mh_frequency, mh_block_frequency, mh_runs, mh_longest_run_of_ones};
    DBG_PRINT("rand self test start\n");
    MHRAND_Prand(rand, sizeof(rand));
    for(i = 0; i < 4; i++)
    {
//        ret = mh_rand_func[i](s, sizeof(s));
...
//        }
        ...
        ret = mh_rand_func[i](rand, sizeof(rand));
//        ret = mh_frequency(rand, sizeof(rand));
        DBG_PRINT("caculate val is: %f\n\n", ret);
        if(ret < 0.01)
        {
            DBG_PRINT("rand self test error\n");
...
//    }
    ...
    p_value = mh_cephes_igamc((double)(maxElements - 1) / 2, V / 2);
//    p_value = mh_cephes_igamc(1.5, 21.2/2);
